...
//        iv.addBlackGradientLayer(frame: self.bounds)
    ...
    private func setup() {
        ...
        NSLayoutConstraint(item: self.videoView, attribute: .top, relatedBy: .equal, toItem: self, attribute: .top, multiplier: 1.0, constant: 0).isActive = true
        NSLayoutConstraint(item: self.videoView, attribute: .bottom, relatedBy: .equal, toItem: self, attribute: .bottom, multiplier: 1.0, constant: 0).isActive = true
        NSLayoutConstraint(item: self.videoView, attribute: .left, relatedBy: .equal, toItem: self, attribute: .left, multiplier: 1.0, constant: 0).isActive = true
        NSLayoutConstraint(item: self.videoView, attribute: .right, relatedBy: .equal, toItem: self, attribute: .right, multiplier: 1.0, constant: 0).isActive = true
    ...
    private func parseData(forSlide slide: ZKCarouselSlide) {
        ...
        if let video = slide.slideVideo {
            self.videoView.layer.addSublayer(video)
        }
